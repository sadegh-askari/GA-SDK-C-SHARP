...
#pragma warning restore 0162
        ...
        public static JSONObject GetEventAnnotations()
        {
            ...
            // Event UUID
            annotations["event_uuid"] = Guid.NewGuid().ToString().ToLowerInvariant();
            // User identifier
            ...
            // remote configs
            if (!GameAnalytics.SettingsGA.UseCustomConfigs)
            {
            ...
            {
                if (GameAnalytics.CustomConfigs is {Count: > 0})
                {
                    JSONNode conf = new JSONObject();
                    foreach (var config in GameAnalytics.CustomConfigs)
                    {
                        conf.Add(config.Name, config.Value);
                    }

                    annotations["configurations"] = conf;
                }
            }
            ...
            //Debug.LogError("Event: " + annotations.ToString());
            return annotations;
        }
